...
from attention_fwd_triton2 import forward
...
from geak_eval.data.TritonBench.data.TritonBench_G_v1.attention_fwd_triton2 import forward as forward_ref

class performance_metrics(Performance_Metrics):
    def __init__(self, dtype=None, is_backward=False, **kwargs):
        super().__init__('attention_fwd_triton2', dtype=dtype, is_backward=is_backward, **kwargs)
        
    def get_input_tensors(self):
        self.input_tensors = []
        for i in range(2, 10):  # Example sizes, adjust as needed
            size = 2 ** i
            q = torch.rand((32, 8, size, 256), dtype=torch.float16)
            k = torch.rand((32, 8, size, 256), dtype=torch.float16)
            v = torch.rand((32, 8, size, 256), dtype=torch.float16)
