...
from context_attn_bloom import context_attention_fwd
...
from geak_eval.data.TritonBench.data.TritonBench_G_v1.context_attn_bloom import context_attention_fwd as context_attention_fwd_ref

class performance_metrics(Performance_Metrics):
    def __init__(self, dtype=None, is_backward=False, **kwargs):
        super().__init__('context_attn_bloom', dtype=dtype, is_backward=is_backward, **kwargs)
        
    def get_input_tensors(self):
        self.input_tensors = []
        for i in range(4, 12):  # Adjust the range according to the expected input sizes
            Z, H, D_HEAD = 4, 16, 256
            N_CTX = 2 ** i
            dtype = torch.float16
    ...
    def call_op_ref(self, input_tensor):
        q, k, v, o, b_req_idx, b_start_loc, b_seq_len, b_prompt_cache_len, max_input_len, req_to_token_indexs = input_tensor
        # Assuming the reference implementation is similar to the main one
        context_attention_fwd_ref(q, k, v, o, b_req_idx, b_start_loc, b_seq_len, b_prompt_cache_len, max_input_len, req_to_token_indexs)
        return o

    def get_gbps(self, input_tensor, runtime):
        q, k, v, o, *_ = input_tensor
    ...
    def get_tflops(self, input_tensor, runtime):
        ...
        # num_operations = 2 * q.size(0) * q.size(1) * q.size(2) * q.size(3)  # Simplified FLOP count
        num_operations = 2 * q.size(0) * q.size(1) * q.size(2) * q.size(2)
