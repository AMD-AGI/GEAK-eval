...
from destindex_copy_kv2 import destindex_copy_kv

import torch
import triton
import triton.language as tl
from geak_eval.data.TritonBench.data.TritonBench_G_v1.destindex_copy_kv2 import destindex_copy_kv as destindex_copy_kv_ref
from geak_eval.perf.performance_utils import Performance_Metrics, do_bench_config

class performance_metrics(Performance_Metrics):
    def __init__(self, dtype=None, is_backward=False, **kwargs):
        super().__init__('destindex_copy_kv2', dtype=dtype, is_backward=is_backward, **kwargs)
        
    def get_input_tensors(self):
        self.input_tensors = []
        for i in range(2, 18):  # Adjust range as needed for testing
            seq_len = 2 ** i
            head_num = 16  # Example head number
    ...
    def call_op_ref(self, input_tensor):
        K, DestLoc, Out = input_tensor
        destindex_copy_kv_ref(K, DestLoc, Out)
        return Out
    ...
    def get_tflops(self, input_tensor, runtime):
        K, DestLoc, Out = input_tensor
        FLOPS = 2 * K.numel()  # Assuming each element involves a load and store operation
