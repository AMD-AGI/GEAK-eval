...
class TestAllCloseEvaluatorTBG(BaseEvaluator):
    ...
    def get_gen_fpath(self, root :str, fname :str) -> str:
        ...
        """
        tmp_gen_folder = os.path.join(root, Names.GEN_FOLDER)
...
class TestAllCloseEvaluatorROCm(TestAllCloseEvaluatorTBG):
    ...
    def _check_match(self, gen_fpath :str, ref_fpath:str , atol :float=1e-1, rtol :float=1e-1, timeout :int =2*60) -> tuple[bool, str, str]:
        """
        Executes the generated file and checks its correctness against the reference file.
        """
    ...
    def execute(self, code :str, log_root:str, exec_root:str, fname:str, atol :float=1e-1, rtol:float=1e-1, timeout :int =2*60, custom_tests_path=None, verbose :bool =False) -> tuple[bool, bool, str, str]:
