...
from .tb_profiler import get_profilers
...
def eval(args):
    ...
    profiler = get_profilers[args.dataset]()
    ...
    for file in tqdm(files, desc="Processing folder", unit="file"):
        ...
        ## Do the profiling
        try:
            correct_py_files = [os.path.join(exec_root, f) for f in os.listdir(exec_root) if f.endswith('.py')]
            print(f"Found {len(correct_py_files)}: {correct_py_files} files in {exec_root} for profiling.")
            if len(correct_py_files) > 0:
                for correct_py_file in correct_py_files:
                    if not os.path.exists(correct_py_file):
                        continue
                    profile_status, stdout_profile, stderr_profile, stdout_analyze = profiler.run(
                        code_or_path=correct_py_file,
                        fname=fname,
                        py_folder=exec_root,
                        target_gpu="MI300", 
                        temp_root=exec_root,
                    ...
                    log = f"{get_time()} => File: {correct_py_file}, Profile Status: {profile_status}, stdout: {stdout_profile}, stderr: {stderr_profile} + '\n' + {stdout_analyze} \n {stdout_analyze}"
                    logs.append(log)
                    profile_path = os.path.join(exec_root, f"{os.path.basename(correct_py_file)}_profile.json")
                    with open(profile_path, 'w') as profile_f:
                        profile_data = {
                            "profile_status": profile_status,
                            "stdout_profile": stdout_profile,
                            "stderr_profile": stderr_profile,
                            "stdout_analyze": stdout_analyze
                        }
                        json.dump(profile_data, profile_f, indent=4)
                    print("-"*80)
                    print(f"Saved profiling data to {profile_path}")
                    print("-"*80)
            else:
                print(f"No valid Python files found in {exec_root} for profiling.", file=sys.stderr)
                profile_status, stdout_profile, stderr_profile, stdout_analyze = None, None, "No valid Python files found for profiling", None
        except Exception as e:
