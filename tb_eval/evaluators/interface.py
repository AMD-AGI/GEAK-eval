...
from ..helpers import get_temp_file
from ..helpers.helper import run_shell, process_code
from ..processors.llm import LLMOutputProcessor
from ..constants import REPO_ROOT, TMP_ROOT, TBG_DATA_ROOT, ROCm_ROOT, Names
...
class TestAllCloseEvaluatorROCm(TestAllCloseEvaluatorTBG):
    def __init__(self, ground_truth_root :str = ROCm_ROOT, MODULE_DIR :str =_MODULE_DIR) -> None:
        super().__init__("CorrectnessWithAllCloseEvaluator")

        self.ground_truth_root = ground_truth_root
        self.MODULE_DIR = MODULE_DIR
        
        self.tmp_gen_folder = os.path.join(self.temp_root, Names.GEN_FOLDER)
        os.makedirs(self.tmp_gen_folder, exist_ok=True)
