...
class PerformanceEvalROCm(BasePerfEval):
    ...
    def evaluate(self, exec_folder: str, gen_perf_folder: str = None, golden_metrics_folder: str = None) -> dict:
        ...
        for py_file_name in tqdm(py_files, desc="Running pytest on ROCm py files"):
            ...
            print(f"{get_time()} Running command: {' '.join(cmd_pytest)} with environment variable PERF_OUTPUT_DIR={gen_perf_folder_abs}")
            pytest_status, pytest_stdout, pytest_stderr = run_shell(cmd_pytest, env=test_env)
            print(f"{get_time()} Pytest execution completed for {py_file_name} in {time.time() - start_time } seconds ")
